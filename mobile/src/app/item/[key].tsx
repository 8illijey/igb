...
function SeoHead({ item, today, unit }: { item: SeoItem; today?: number | null; unit?: string }) {
  ...
  // 제목에 값과 '오늘 시세'를 함께 넣는다.
  //  · 값이 있어야 검색 결과에서 클릭 전에 가격을 안다.
  //  · '시세'가 있어야 '배추 가격/시세' 같은 검색어와 맞물린다.
  // 30자를 넘는 품목이 10개 있지만 잘리는 건 뒤의 브랜드뿐이라 품목·가격·시세는 다 남는다.
  // 가격이 없는 품목(조사 중단 등)은 값만 뺀다.
  const title =
    p != null ? `${item.name} ${won(p)}원 · 오늘 시세 | 이거비싸?` : `${item.name} 오늘 시세 | 이거비싸?`;
  ...
  return (
    <View style={styles.flowCard}>
      ...
      {ready ? (
        ...
        // 제철 품목뿐 아니라 신규 편입 품목(2026-09-01 블루베리·현미 등 7종)도 이 분기를 탄다 —
        // '제철에만 조사돼'는 현미에겐 틀린 설명이라 중립 문구(2026-09-06 제보).
        <Text style={styles.flowCaption}>
          아직 한 해 흐름을 보여줄 조사 자료가 부족해요. 자료가 쌓이는 대로 채워드릴게요.
        </Text>
      ) : (
        <>
