 ...
 *
 * ══════════════════════════════════════════════════════════════════════════════
 *  결론부터 — 이 파일은 지금 **보험으로만** 유지한다 (2026-09-17 결정)
 * ══════════════════════════════════════════════════════════════════════════════
 *
 *  ❗ 화면에 보이는 '이맘때 평균'은 이 파일이 아니라 **KAMIS 공식 dpr7**이다.
 *     (2026-09-17 실측: 네 카테고리 124행 전부 `day7: "일평년"` 라벨이 붙은 KAMIS 원본 응답)
 *     여기 계산값은 그 공식값의 **근사치**이지 대체재가 아니다. 절대 바꿔 끼우지 마라.
 *
 *  ── 왜 만들었나 (2026-08-18) ────────────────────────────────────────────────
 *  당시 KAMIS 원본이 데이터센터 IP를 막아서(2026-07~) 워커·CI가 dpr7을 아예 못 받았다.
 *  임시로 워커가 verdicts의 months[이번달]을 dpr7 자리에 넣었는데, 그 verdicts가 다시 워커를
 *  거쳐 만들어지므로 **순환**이었다 — 56개 품목 중 50개에서 normal === months[이번달]였다.
 *  즉 앱이 '평년'이라 부르던 값이 실은 자체 최근 1년 평균이었다. 이걸 끊으려고, 미러가 정상적으로
 *  주는 2020년 이후 원천 가격만으로 평년을 KAMIS 정의 그대로 다시 계산한 것이 이 스크립트다.
 *
 *  ── 왜 지금은 안 쓰나 (2026-09-17) ─────────────────────────────────────────
 *  그 사이 KAMIS가 복구됐다. 워커는 원래부터 KAMIS를 1순위로 부르므로(worker/src/index.ts),
 *  지금은 공식 dpr7이 그대로 들어온다. 만든 이유였던 '받을 수 없음'이 사라진 것이다.
 *  그리고 우리 계산은 공식값과 절대오차 중앙값 3.2%로 어긋난다 — 받을 수 있는 공식값을 두고
 *  근사치를 쓸 이유가 없다.
 *
 *  ── 그럼 왜 지우지 않고 남기나 ─────────────────────────────────────────────
 *  ① 보험. KAMIS가 또 막히면 워커가 자동으로 이걸 쓴다(worker/src/index.ts `baselineOf`).
 *     2026-07~08에 실제로 겪은 일이라 가상의 위험이 아니다. 이게 없으면 그때 앱 홈이 통째로 빈다.
 *  ② 월별 계절 패턴(`normalMonths`)의 유일한 출처. dpr7은 **요청한 날짜 하나의 값 한 개**라
 *     12개월 곡선을 만들려면 품목당 12~365회를 물어야 한다(90품목 → 1,080~32,850회).
 *     여기선 과거치를 한 번에 받아 365칸을 통째로 만든다.
 *     단 이 용도의 소비처인 '가격이 오르는 시기' 문단은 2026-09-17 현재 미배포다.
 *
 *  ── 유지 비용 ──────────────────────────────────────────────────────────────
 *  주 1회 CI(.github/workflows/baselines.yml, 월요일 04:40 KST) + 산출물 337KB.
 *  라이브 화면에 닿는 곳은 현재 0곳. 비용을 알고 감수하는 선택이다.
 *
 *  ── 남은 함정 ──────────────────────────────────────────────────────────────
 *  이 값과 KAMIS dpr7은 같은 개념의 다른 숫자다. **한 화면에서 둘을 같이 금액으로 쓰지 마라.**
 *  사용자에게 같은 뜻의 숫자가 두 개로 보인다(양배추 기준 4,304원 vs 3,936원, 9% 차이).
 *
 *  현재 무엇이 어디서 오는지: `cd mobile && npm run check-live`
 *  아래 '왜 만들었나'의 세부 기록은 2026-08-18 시점이며 지금 상태가 아니다.
 *  이 주석을 현재 상태로 읽고 잘못된 변경을 제안한 사고가 2026-09-17에 있었다(AGENTS.md 참고).
 *
 * 왜 만들었나 [2026-08-18 기록 — 당시 상태이며 지금은 다르다]
 *  KAMIS 원본이 데이터센터 IP를 막아서(2026-07~) 워커·CI가 직접 못 받던 시기가 있었다. 그때는
 *  공공데이터포털 미러로 돌았는데, 미러는 '조사된 원천 가격'만 주고 파생값(dpr7 평년)이 없었다.
 *  임시로 워커가 verdicts의 months[이번달]을 dpr7에 주입했는데 그 verdicts가 다시 워커를 통해
 *  만들어지므로 순환이었다 — 당시 56개 품목 중 50개에서 normal === months[이번달].
 *  → 이 순환은 2026-08-18 커밋 4e13bfb로 제거됐고, KAMIS도 그 뒤 복구됐다.
 *
 *  미러는 2020년치까지 원천 가격을 정상적으로 준다. 그래서 평년을 KAMIS 정의 그대로 계산할 수 있다.
 *
 *  실측 정확도(2026-08-18, 15품목을 KAMIS 실제 dpr7과 대조):
 *    이 방식 절대오차 중앙값 3.2%, 5% 이내 67%
 *    당시 순환값        절대오차 중앙값 8.1%, 5% 이내 33%   (찹쌀 +41%, 오이 +34%, 양배추 -21%)
 *  → 3.2%는 '우리 계산이 공식값과 이만큼 다르다'는 뜻이다. 공식값을 이걸로 대체할 근거가 아니다.
 *
