...
## 반드시 지킬 것 — 쿠팡 파트너스 공식 호출 한도

**이 표가 기준이다. 어떤 이유로도 넘기지 마라.**
...
한도를 넘기면 경고가 쌓이고, **3회 초과하면 계정 이용이 제한된다.**

> 2026-08-17 최초 201개 일괄 생성 중 2회 초과 → 24시간 차단(다음날 16:13까지)을 실제로 맞았다.
> 2026-09-09에 또 한 번 — 유력 원인은 CLI+REPL 중복 동시 실행(35×2=70/분).
> 남은 한도는 **1회**뿐이라고 생각하고 다뤄야 한다.

주의할 지점 두 가지:

- **합산 한도(100/분)는 종류별 한도와 따로 걸린다.** 검색 50 + 링크생성 50을 각각 지켜도
  동시에 돌리면 합산에서 걸린다.
- **동시 실행이 가장 흔한 사고 경로다.** 한도는 계정 단위라 프로세스를 나눠도 합산된다.
  스크립트에 실행 잠금이 있는 이유가 이것이다.

그래서:

- 스크립트는 기본 **RPM 20**으로 스스로를 늦춘다 (`DEFAULT_RPM`). 이 값을 올리지 말 것.
  검색과 링크생성이 겹쳐도 40 < 50이고, 혹시 둘이 동시에 돌아도 합산 40 < 100이다.
- `MAX_SAFE_RPM = 20`이 하드 클램프다. `--rpm=50`을 줘도 20으로 강제 하향된다.
  **이 상수를 올리는 변경은 하지 마라.**
