...
> **상태: 구현 완료 — 단, 3개 목표 중 2개 (2026-08).** 아래는 당시 계획 원문이고
> 코드 스니펫은 commit bf73639 시점의 "before"다.
> [2026-09-17 확인] `Sparkline.tsx`에 `hintShown` 세션 가드, `hintX`/`hintOpacity` 스윕,
> 인덱스 변경 시 `Haptics.selectionAsync()`가 들어가 있다.
> **툴팁·크로스헤어 등장 페이드는 의도적으로 구현하지 않았다** — 문서 맨 아래 실행 노트에 이유가 있다.
