...
(function () {
  ...
  // 한 번 호출로 받는 검색 결과 수. 호출 횟수는 그대로이므로 넉넉히 받는 편이 항상 낫다.
  // 36이면 상위권을 특정 품종이 점령했을 때 정작 필요한 상품이 안 보인다 —
  // 2026-08-24 방울토마토는 상위 36건 중 35건이 대추방울이라 후보가 1개뿐이었다.
  // 60으로 올리면 같은 검색에서 후보가 4개로 늘어난다.
