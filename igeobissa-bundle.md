# 이거비싸? (IGB) — 프로젝트 전체 번들 [2026-06-13 스냅샷 · 현행 아님]

> ⛔ **이 파일을 현재 사양으로 읽지 마라.** 2026-06-13 시점의 product.md·design.md·앱 소스를
> 통째로 복사해 굳힌 기록물이다. 그 뒤로 코드도 디자인도 여러 번 바뀌었고 이 파일은 따라가지 않는다.
> 갱신하지도 마라 — 스냅샷은 그때 그대로여야 의미가 있다.
>
> **현행 사양은 여기에 있다**
> - 제품 [product.md](product.md) · 디자인 [design.md](design.md) · 작업 규칙 [AGENTS.md](AGENTS.md)
> - 앱 소스 `mobile/src` (이 파일에 박힌 소스 전문은 3개월 낡았다)
> - 라이브 동작 `cd mobile && npm run check-live`
>
> 알려진 불일치 예 — 이 파일은 "신호 카드 좌측 2px accent rail"을 살아 있는 규칙으로 서술하지만
> (368·373·524·664행), 그 패턴은 2026-07-05에 폐기됐다. design.md 283행 참조.
