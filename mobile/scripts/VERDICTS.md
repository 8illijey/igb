...
## 판정 기준값(`normal`)이 어디서 오나

`build-verdicts.mjs`는 KAMIS 응답의 **`dpr7`(일평년)을 그대로** 쓴다. 우리가 계산한 값이 아니다.
워커가 KAMIS 원본을 1순위로 부르므로 평상시엔 KAMIS 공식 평년이 들어온다.
원본이 죽으면 워커가 `baselines.json`(우리 근사치, 오차 중앙값 3.2%)을 대신 주입한다.

**`baselines.json`은 `dpr7`의 대체재가 아니라 대타다.** 둘은 품목별로 몇 % 다르므로
화면에서 금액으로 나란히 쓰면 안 된다. `baselines.json`의 본래 역할은 월별 계절 패턴
(`normalMonths`)으로, KAMIS가 월별 평년을 주지 않아 필요하다.

지금 어느 경로로 도는지: `npm run check-live`
(이 구분을 놓쳐 2026-09-17에 잘못된 전환을 제안한 사고가 있었다. [AGENTS.md](../../AGENTS.md) 참고)
